...
def segmentation_scores(label_trues, label_preds, n_class, ignore_index=None):
...
def hd95_list(label_gt, label_pred, n_class, ignore_index=None, return_matrix=False):
    assert len(label_gt) == len(label_pred)
    batchSize = len(label_gt)
    hd95 = np.zeros((batchSize, n_class), dtype=np.float32)
    for batch_id, (l_gt, l_pred) in enumerate(zip(label_gt, label_pred)):
        for class_id in range(n_class):
            img_A = l_gt == class_id
            if 0 == np.count_nonzero(img_A):
                score = np.nan
            else:
                img_B = l_pred == class_id
                if 0 == np.count_nonzero(img_B):
                    score = np.inf
                else:
                    score = metric.binary.hd95(img_B, img_A)
            hd95[batch_id, class_id] = score

    if return_matrix:
        return hd95
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        return np.nanmean(hd95, axis=0)

def dice_score_list(label_gt, label_pred, n_class, ignore_index=None, return_matrix=False):
    """
    ...
    for batch_id, (l_gt, l_pred) in enumerate(zip(label_gt, label_pred)):
        if ignore_index is not None:
            mask = l_gt == ignore_index
            l_pred[mask] = ignore_index
        for class_id in range(n_class):
            img_A = np.array(l_gt == class_id, dtype=np.float32).flatten()
            if np.count_nonzero(img_A) == 0:
                dice_scores[batch_id, class_id] = np.nan
            else:
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        return np.nanmean(dice_scores, axis=0)
